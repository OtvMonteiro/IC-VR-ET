...
public class heatmapDOTS : MonoBehaviour
{
   ...
   [SerializeField] public bool populate = false;
    public GameObject heatmapObjectPrefab; //Prefab (instanciating heatmap Elements via GameObject conversion)
    private bool background_update = true;
    private Entity heatmapObject;
        ...
        void Start()
    {
        ...
        heatmapObject = GameObjectConversionUtility.ConvertGameObjectHierarchy(heatmapObjectPrefab, settings);
        //Add identifier component to facilitate identification by Systems
        entityManager.AddComponent<heatmapIdentifier>(heatmapObject);
    ...
    void Update()
    {
        if(enableHeatmap && background_update){
          //Captures point (simple, dwell or wait)
            ...
            if(Vector3.Distance(currentPoint, lastPoint)> HeatmapPointsScale){
                lastPoint = currentPoint;
                createEntities(currentPoint);
                if(populate) Populate(currentPoint);
            }
    ...
    private void createEntities(Vector3 point){
        ...
        var instance = entityManager.Instantiate(heatmapObject);
    ...
    private void Populate (Vector3 point){
        for (int i = 0; i < 10; i++)
        {
            Vector3 pop_point = point + new Vector3(Random.value*HeatmapPointsScale*3,
                                                    Random.value*HeatmapPointsScale*3,
                                                    Random.value*HeatmapPointsScale*3);
    ...
    public IEnumerator dwellPoints()
    {
        ...
        Vector3[] dwell_points = new Vector3[n_points];
        ...
        for(int n=0; n<n_points; n++)
        {
            ...
            yield return new WaitForSecondsRealtime(1/(pointF_Hz*n_points));
        }

        centerOfMass = centerOfMass/n_points;
        bool closeEnough = true;

        //Verify if the points are close enough to register
        for(int n=0; n<n_points && closeEnough ; n++)
        {
            if(Vector3.Distance(centerOfMass, dwell_points[n]) < pointF_Hz/(n_points*HeatmapPointsScale)){}
